            ...
            <div class="item-wrap dropdown">
              ...
              <div class="dropdown-menu" v-if="user.data.enterprises && user.data.enterprises.length > 0">
                ...
                <ul>
                  ...
                  <li class="menu-item" v-for="en in sortEnterprises" v-if="en.uu!=enterprise.uu">
                    <a @click="switchEnterprise(en)" :title="en.enName">{{ en.enName }}</a>
                  </li>
                  <li class="menu-item" v-if="enterprise.uu">
                    <a @click="switchEnterprise({uu: 0})">
                      <span v-text="user.data.userName"></span>（个人账户）</a>
                  </li>
                </ul>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    sortEnterprises() {
      let ens = deepCopy(this.user.data.enterprises)
      if (ens && ens.length) {
  ...
  methods: {
    ...
    onLoginClick() {
      this.$http
        .get('/login/page', { params: { returnUrl: window.location.href } })
        .then(response => {
          if (response.data) {
            window.location.href =
...
.header
  ...
  li
  span,
.header .navbar .navbar-container .navbar-right .dropdown .dropdown-menu li a {
  font-size: 12px;
}
.header
  .navbar
