define(['app/app'], function(app) {
	app.register.controller('addressAdminCtrl', ['$scope', '$anchorScroll', '$location', 'ShippingAddress', 'ngTableParams', 'BaseService', '$modal', 'toaster', function($scope, $anchorScroll, $location, ShippingAddress, ngTableParams, BaseService, $modal, toaster) {
		...
		var loadAddrs = function() {
			getMethod();
			ShippingAddress.getListEnterprise({ship: !$scope.isSendType}, function(data) {
	...
	app.register.controller('editAddrCtrl', ['$scope', 'isSetTop', 'addr', 'isSendType', '$modalInstance', 'toaster', '$http', 'ShippingAddress', function($scope, isSetTop, addr, isSendType, $modalInstance, toaster, $http, ShippingAddress){
	    ...
	    $scope.save = function () {
	    	...
	    	// send属性 控制本地址是否是发货地址
	    	ShippingAddress.save({isSetTop: $scope.isSetTop, send: $scope.isSendType, isPersonal: false}, address, function(data){
