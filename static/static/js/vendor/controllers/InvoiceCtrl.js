define(['app/app'], function(app) {
	'use strict';
	app.register.controller('InvoiceCtrl', ['$scope', 'toaster', 'InvoiceFPurchase', 'ChineseToPinYin', 'Order' , '$modal', '$stateParams', 'BaseService', 'ngTableParams', 'ComponentActive', '$rootScope', '$window', '$filter', '$state', 'Purchase', function($scope, toaster, InvoiceFPurchase, ChineseToPinYin, Order , $modal, $stateParams, BaseService ,ngTableParams ,ComponentActive, $rootScope, $window, $filter, $state, Purchase) {
		var enIdFilter = $filter('EncryptionFilter');
		//如果window.sessionStorage 有值，就设置为当前的active值
		$scope.active = window.sessionStorage.getItem('invoiceState')? window.sessionStorage.getItem('invoiceState'):'tobeshipped';
...
//					state = '405'; break;
		...
		$scope.setActive = function(state) {
			window.sessionStorage.setItem('invoiceState',state);
		...
		$scope.send = function(invoice){
			...
			window.sessionStorage.setItem("orderAdmin", "invoice");
		};
	}]);
