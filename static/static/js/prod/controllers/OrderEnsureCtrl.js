	...
	app.register.controller('OrderEnsureCtrl', [ '$scope', '$stateParams', '$location', '$modal', '$filter', 'toaster', 'Order', 'OrderSimpleInfo', 'Cart', 'ShippingAddress', 'PickUpAddress' , 'SessionService', '$window' ,'Bill', 'bankInfoService', '$interval', 'ExpressPrice', '$q', '$rootScope', 'Ysepay', 'AuthenticationService', 'BaseService', function($scope, $stateParams, $location, $modal, $filter,  toaster, Order, OrderSimpleInfo , Cart, ShippingAddress, PickUpAddress, SessionService, $window, Bill, bankInfoService,  $interval, ExpressPrice, $q, $rootScope, Ysepay, AuthenticationService, BaseService) {
		...
		$scope.showDropdown = function(item) {
			switch (item) {
				...
				case 'selectCurrency': $scope.dropdownState.selectCurrency = !$scope.dropdownState.selectCurrency;
                    $(window).scroll(); // 触发一个滚动事件，使锁表头指令生效，从而设置表头为固定定位，从而下拉框不会受到祖先容器的overflow-y的影响
                    break;
			}
		};
		...
		var checkCanSubmitOrderOrNot = function() {
            // 币别与地址不一致时的错误提示
            delete $scope.currencyInconsistentWithAddr;
			if($scope.order && $scope.selectAddress) {
				for(var i = 0; i < $scope.order.orderDetails.length; i++) {
					...
					if($scope.order.currency == 'USD' && detail.goodsHistory.currencyName == 'RMB') {
					    $scope.currencyInconsistentWithAddr = '香港收货地址只支持美元，不支持'+'订单明细' + i+1 + '的币别为RMB';
                        $scope.canSubmitOrder = false; return;
                    }
                    if($scope.order.currency == 'RMB' && detail.goodsHistory.currencyName == 'USD') {
                        $scope.currencyInconsistentWithAddr = '大陆收货地址只支持人民币，不支持'+'订单明细' + i+1 + '的币别为USD';
                        $scope.canSubmitOrder = false; return;
                    }
                    if($scope.order.currency == 'RMB' && $scope.selectAddress.area.startsWith('香港')) {
                        $scope.currencyInconsistentWithAddr = '当前商品只支持人民币购买，请选择大陆收货地址';
                        $scope.canSubmitOrder = false; return;
                    }else if($scope.order.currency == 'USD' && !$scope.selectAddress.area.startsWith('香港')) {
                        $scope.currencyInconsistentWithAddr = '当前商品只支持美元购买，请选择香港收货地址';
                        $scope.canSubmitOrder = false; return;
                    }
				}
		...
		var initOrder = function() {
			...
			// 默认选择在线支付  --1102在线支付
			if(!$scope.order.paytype) $scope.order.paytype = 1102;
		};
		...
		// 得到满足指定币别的第一个地址或最后一个地址
        var getAddrByCurrency = function (currency, reverse) {
            if(!reverse) { // 正序
                ...
                var addr;
                for(var i = $scope.addrs.length - 1; i >= 0; i--) {
                    addr = $scope.addrs[i];
                    if(currency == 'RMB') {
                        if(!addr.area.startsWith("香港")) {
                            return addr;
        ...
        var setDefaultAddr = function(isLast) {
            if ($scope.addrs.length > 0 && $scope.order && $scope.order.currency) {
                var prevAddrId= $scope.selectAddress ? $scope.selectAddress.id : undefined;// 之前选择的地址的id，修改该地址后应该仍然选中该地址
                $scope.selectAddress = getAddrByCurrency($scope.order.currency, isLast); // 设置地址
		...
		// 更新金额
		var refreshAmount = function (detail) {
			detail.amount = detail.number * detail.taxUnitprice;
		};
		...
		// 线下支付
		var offlinePay = function () {
			$modal.open({
				templateUrl : 'static/view/prod/modal/offline-pay-modal.html',
				...
				resolve: {
					...
					totalAmount: function () {
					    if($scope.order.ensurePrice) {
					        return $scope.order.ensurePrice;
                        }else {
                            return $scope.productsAmount + $scope.importExtraCharges;
                        }
                    }
				}
			}).result.then(function(success){
				toaster.pop('success', '提示 ' + '您已成功完成线下支付，即将为您跳转到订单跟踪页');
                window.location.href = BaseService.getModulePathname(window.location.pathname, 'product', 'user') + '#/home/myOrder_todo?state=tobepaid';
			}, function(cancel){
				toaster.pop('info', '提示 ' + '您已取消线下支付');
                window.location.href = BaseService.getModulePathname(window.location.pathname, 'product', 'user') + '#/home/myOrder_todo?state=tobepaid';
			});
		}
	}]);
