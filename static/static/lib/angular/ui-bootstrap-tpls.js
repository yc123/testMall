 ...
 * License: MIT
 * typeahead-min-length 已修改 支持触发条件：最小输入字符数大于等于0  author：孙土桂
 */
...
.controller('CarouselController', ['$scope', '$interval', '$animate', function ($scope, $interval, $animate) {
  ...
  self.currentSlide = null;
    ...
    var setCarousel = function () {
        var backgroundColor = $('.carousel .active-add .banner-background-color').val();
        if(!backgroundColor) {
            backgroundColor = $('.carousel .active .banner-background-color').val(); // 页面加载时获取第一个幻灯片背景颜色
        }
        $('#banner').css('background-color', backgroundColor);
    };

    // 根据下一个幻灯片索引设置幻灯片背景颜色
    var setCarouselByNextIndex = function(nextIndex) {
        var nextDataHolder = $($('.carousel .banner-background-color').get(nextIndex));
        var backgroundColor = nextDataHolder.val(); // 页面加载是，angularjs为完成编译页面，因此无法获取第一张幻灯片background-color
        $('#banner').css('background-color', backgroundColor);
    };

    // 设置页面加载时设置第一张幻灯片背景颜色
    var setBgColorWhenPageLoading = function () {
        var style = $('#banner').attr('style');
        if(!style || style.indexOf('background-color') == -1) {
            var backgroundColor = $('.carousel .active .banner-background-color').val(); // 页面加载时获取第一个幻灯片背景颜色
            $('#banner').css('background-color', backgroundColor);
        }
    };
  ...
  self.select = $scope.select = function(nextSlide, direction) {
    ...
    function goNext() {
      ...
      angular.extend(nextSlide, {direction: direction, active: true}); // 让下一张幻灯片触发
      setCarouselByNextIndex(nextIndex);
      angular.extend(self.currentSlide || {}, {direction: direction, active: false}); // 取消上一张幻灯片的active状态
      if ($animate.enabled() && !$scope.noTransition && nextSlide.$element) {
        $scope.$currentTransition = true;
        nextSlide.$element.one('$animate:close', function closeFn() {
          $scope.$currentTransition = null;
          // setCarousel(); // 这里是幻灯片已经切换之后
          setBgColorWhenPageLoading();
        });
      }
...
angular.module("template/tabs/tab.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tabs/tab.html",
    "<li ng-class=\"{active: active, disabled: disabled}\" ng-mouseover=\"\">\n" +
